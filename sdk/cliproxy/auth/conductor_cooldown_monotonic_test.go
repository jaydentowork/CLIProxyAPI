...
func TestManager_MarkResult_CredentialScope_DoesNotPromoteSiblingDeadlineToQuota(t *testing.T) {
	...
	// 1. Model B gets a 404 (notFoundCooldown: 10m deadline; explicit model-not-found would be 12h).
	m.MarkResult(context.Background(), Result{
		AuthID: auth.ID, Provider: auth.Provider, Model: "model-b",
	...
	// Model B's NextRetryAfter should still be the 10m 404 cooldown set in step 1.
	if bState1.NextRetryAfter.Before(before.Add(9 * time.Minute)) {
	...
	// Model A must NOT have been elevated to model-b's 10m 404 deadline.
...
func TestManager_ApplyAuthFailureState_PreservesLongerCredentialDeadline(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			m, auth := newCooldownMonotonicManager(t, "model-a")
			// Credential-wide 404 (notFoundCooldown: 10m deadline).
			m.MarkResult(context.Background(), Result{
				AuthID: auth.ID, Provider: auth.Provider, Model: "",
