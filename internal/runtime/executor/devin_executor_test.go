...
func TestParseInteractionsPayload_NamespaceFunctionDeclarations(t *testing.T) {
	...
	// The responses-to-interactions translator flattens namespace children into
	// qualified flat tools, so all three probes must reach the Devin payload.
	if got := gjson.GetBytes(interactionsPayload, "tools.#").Int(); got != 3 {
		t.Fatalf("translated tools = %d, want 3; payload=%s", got, interactionsPayload)
	}
	...
	wantNames := []string{"flat_probe", "diagnostic__echo_probe", "diagnostic__second_probe"}
	...
	wantEchoParams := gjson.GetBytes(interactionsPayload, "tools.1.parameters").Raw
	if string(tools[1].Parameters) != wantEchoParams {
		t.Fatalf("echo probe parameters = %s, want %s", tools[1].Parameters, wantEchoParams)
	}
	wantSecondParams := gjson.GetBytes(interactionsPayload, "tools.2.parameters").Raw
